...
public class XinxiService extends SimpleBizservice<XinxiMapper>{
   ...
   public  List<Xinxi> findXinxi(int lanmuId){
	...
	public List<SearchVo>  search(String title){
   	   List<Xinxi> listXinxi= getEntity(" where title like '%"+title+"%'");
		if(listXinxi==null)
			return null;
		List<SearchVo> listSearch=new ArrayList<>();
		for(Xinxi xinxi :listXinxi){
			SearchVo searchVo=new SearchVo();
			searchVo.setId(xinxi.getId());
			searchVo.setName(xinxi.getTitle());
			searchVo.setHref("/e/xinxiinfo.jsp?id="+ xinxi.getId());
			searchVo.setTupian(xinxi.getTupian2());
			searchVo.setXtype(1);
