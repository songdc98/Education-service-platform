...
jQuery.fn.play = function(){
	...
	$('#ap-console-play').children('i').removeClass('icon-play');
	$('#ap-console-play').children('i').addClass('icon-pause');
	this[0].play();
}
jQuery.fn.pause = function(){
	$('#ap-player-hide').css('background','rgba(0,0,0,0.5)');
	$('#ap-player-icon').css('opacity','1');
	$('#ap-console-play').children('i').addClass('icon-play');
	$('#ap-console-play').children('i').removeClass('icon-pause');
	this[0].pause();
}
...
// --------------------------------------------------------------------------
	...
	$('#ap-console-full').click(function(){
		if ($('#ap-player-bar').css('position') == 'absolute') {
			...
			$('#ap-console-full').children('i').removeClass('icon-check-empty').addClass('icon-fullscreen');
			exitFullscreen();
			setJdbar(apcore,10);
		}else {
			ap_fullscreen = true;
			$('#ap-player-bar').css({width:$(window).width()+'px',height:$(window).height()+'px',position:'absolute',top:"0px",left:"0px"});
			$('#ap-console-full').children('i').removeClass('icon-fullscreen').addClass('icon-check-empty');
			fullscreen();
			setJdbar(apcore,10);
			setTimeout(function() {
				setJdbar(apcore,10);
